...
def conditionalformatting(worksheet):
    ...
    #Colour 1
    worksheet.conditional_format(0, 7, max_row, max_col - 1, {'type':     'cell',
                                            'criteria': 'between',
                                            'minimum':    0,
                                            'maximum':    50.99,
                                            'format': format1})
    ...
    worksheet.conditional_format(0, 7, max_row, max_col - 1, {'type': 'blanks',
                                            'stop_if_true': False,
                                            'format': format5})
...
for row in processeddata:
    ...
    if 'Scope:' in row[0]: #if the row starts with scope then we know we have reached a new tutor group and we need to create a new folder
        ...
        for i in range(0,len(row),1):  #for each register mark and empty space in the student data row
            ...
            if i == 0: #if its the row element containing the name
                ...
                if entry == '/\\': #if present all day
                    ...
                    if ('C' not in entry) and ('L' not in entry) and ('P' not in entry) and ('V' not in entry) and ('W' not in entry) and ('R' not in entry) and ('Y' not in entry) and ('#' not  in entry) and ('D' not in entry) and ('B' not in entry) and ('E' not in entry) and ('S' not in entry): #these codes should not penalise attendance
                        daycounter = daycounter + 1 #acknowlegde school was open
                    ...
                    if ('LL' not in entry) and ('C' not in entry) and ('P' not in entry) and ('V' not in entry) and ('W' not in entry) and ('R' not in entry) and ('Y' not in entry) and ('#' not in entry) and ('D' not in entry) and ('B' not in entry) and ('E' not in entry) and ('S' not in entry): #these codes should not penalise attendance
                        daycounter = daycounter + 1 #acknowlegde school was open
                        attended = attended + 0.5 # acknowlegde child was in for half a day
                    ...
                    if ('BB' not in entry) and ('C' not in entry) and ('P' not in entry) and ('V' not in entry) and ('W' not in entry) and ('R' not in entry) and ('Y' not in entry) and ('#' not in entry) and ('D' not in entry) and ('L' not in entry) and ('E' not in entry) and ('S' not in entry): #these codes should not penalise attendance
                        daycounter = daycounter + 1 #acknowlegde school was open
                        attended = attended + 0.5 # acknowlegde child was in for half a day
                    ...
                    illness_counter = illness_counter + 1
                    daycounter = daycounter + 1

                elif 'U' in entry: #if present for half a day eg. for unauthorised mornign/afternoon appointments or Us)
                    if ('C' not in entry) and ('P' not in entry) and ('V' not in entry) and ('W' not in entry) and ('R' not in entry) and ('Y' not in entry) and ('#' not in entry) and ('D' not in entry) and ('L' not in entry) and ('E' not in entry) and ('S' not in entry): #these codes should not penalise attendance
                        daycounter = daycounter + 1 #acknowlegde school was open
                    else:
                        daycounter = daycounter + 1
                        attended = attended + 0.5 # acknowlegde child was in for half a day
                    ...
                    flaggedZ= flaggedZ +1
                    if len(relevantinfo) >1: #if student has already attended then flag so that empty spaces are placed after their last attendance (eg. when they are taken off roll) 
                        nanafter = 1
                    else: #if student has not been on roll yet then flag so that empty spaces are later placed before data
                        nanbefore =1
                    
                else: #for any other codes (eg. M, O) just aknowledge the day but dont mark attendance
                    daycounter = daycounter + 1 #acknowlegde school was open
                ...
                if isinstance(previousentry, str) == True: #if this is the first nan column reached between weekly data
                    ...
                    elif daycounter > 0 and flaggedZ > 0: #if student entered roll mid week
        ...
        if notenrolledweek >0 and nanbefore == 1:
        ...
        studentinfo.insert(5,illness_counter) #append number of days taken off ill
        studentinfo.insert(6,n) #insert tab to separate from non-cumulative attendance
        ...
        if nanafter == 1 or nanbefore ==1 : 
            ...
            if nanafter ==1: #if student was on roll and then stopped being on roll
                ...
                insertioncounter = 7 #insert after name of student, cumulative attendance, days off ill and separation tab
...
headers = [{'header': 'Name'},{'header': 'Year'}, {'header': 'Tutor group'}, {'header': '  '}, {'header': 'Cumulative attendance for year-to-date'},{'header': 'Illness days'}, {'header': ' '}]
end3 = len(weeks)
